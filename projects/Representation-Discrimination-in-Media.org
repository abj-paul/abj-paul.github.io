...
1. Use narrow definition and broad definition to define the subjection concept of "indigenous article".
2. Use of LLM to identify indigenous articles.
3. We then estimate news genre distribution using semi-supervised topic modeling.
4. We also estimate news sentiment and average article length to get more data on news representation.
5. Approximate agenda setting from news genre distribution.
6. 5-frame clasification and frame identification.
7. Use word cloud to approximate priming. NO PREVIOUS WORK.
8. FGD
...
*** Paper dumps - Framing Evaluation and Noun encoding
 For that, we consider universal sentence encoder (USE) to encode frames. Because frames are noun phrases, as evident from the frames identified in \cite{morstatter2018identifying} and \cite{pew2009framing}. And according to Ajallouda et al., the best technique that can be used to represent noun phrases is USE \cite{ajallouda2022doc2vec}. Next, we cluster these frames into a hierarchical structure using Agglomerative Clustering, ultimately isolating X top-level frames. We use silhouette score to find the optimal number of clusters to be X. 

 According to \cite{morstatter2018identifying}, framing is inherently nuanced. The intertwined nature of frames often leads to confusion among annotators, as a single news piece can contain multiple frames \cite{del2024human}. This complexity makes it challenging to ensure that all frames are accurately captured during annotation. Additionally, a machine may detect a subset of frames or identify an overlapping but distinct set of frames. This scenario is best treated as a fuzzy or soft evaluation problem. A "soft match" occurs when a machine's prediction is considered correct if it matches any of the multiple frames identified by human annotators, even if the match is not exact or comprehensive. The Jaccard Index is used to estimate similarity between sets or sequences \cite{salvatore2020beware}. And Binary indicators are also used across domains to represent true or false in a relationship between sets \cite{salvatore2020beware}, \cite{baharav2020spectral}. So we use Jaccard Index and Binary Indicator for match, here-forth called Binary Match Indicator as soft evaluation metric. In our case, BMI can be represented as:
    \[
    ...
    This metric represents match or non-match between machine and human annotation. So we can now use regular formula to calculate human-machine agreement rate \cite{del2024human}. In our case, if we evaluate over M articles, we can calculate the Average Agreement Rate using the binary match as follows:
    \[
...
| [[https://drive.google.com/file/d/1lON-jhQ0I8VSz8a1lFO-Lh8niSg6hqGB/view?usp=sharing][General articles]]    | 5000 |
| [[https://drive.google.com/file/d/1MJbsSGJjmWByBJNePJ6ETDs9dLiXLv82/view?usp=drive_link][Indigenous articles]] | 4892 |
** Survey
*** Literature
